...
class kompass(Spider):
	...
	def parse_entrp_details(self, response):
		...
		#selenium_number = get_number(response.url)
		...
		details['get_number'] = response.xpath(".//input[starts-with(@id, 'freePhone-contactCompanyForCompany')]/@value").extract_first()
	...
	def get_number(self, response):
		driver.get(response)
		driver.find_element_by_css_selector(By.linkText("Login")).click()
		# 
		return 
