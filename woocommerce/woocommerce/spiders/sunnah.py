...
class sunnah(Spider):
	...
	def parse_cards1(self, response):

		details = WoocommerceItem()

		# define path
		path_name = '.h1.page-title ::text'
		path_sale_path = '.current-price > span ::attr(content)'
		path_regular_price = '.product-discount > span ::text'
		#path_category = ''
		path_short_description = 'div[itemprop="description"] ::text'
		path_descreption = '#product-infos-tabs-content ::text'
		#path_currency = ''
		path_image = '.col-md-4.col-product-image ::attr(src)'
		...
		details['sale_price'] = str(response.css(path_sale_path).extract_first()).strip().replace('\u20ac\u00a0', '')
		details['regular_price'] = str(response.css(path_regular_price).extract_first()).strip().replace('\u20ac\u00a0', '')
		...
		details['short_description'] = str(
			' '.join(str(e.strip()) for e in response.css(path_short_description).getall())
			)
		details['description'] = str(
			' '.join(str(e.strip()) for e in response.css(path_descreption).getall())
			)
		...
		# if there is no tags "rupture de stock" stock equal 1
		if response.css('#product-availability') is not None:
			details['in_stock'] = 1
		
		try :
			details['sku'] = response.url.split('/')[-1].split('-')[0]
		except Exception as e:
			print('sku error extraction ',e)
